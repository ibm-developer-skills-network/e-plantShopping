...
import { addItem, removeItem } from './CartSlice';// Import the reducers from CartSlice
import './ProductList.css'; // Import your custom CSS
...
function ProductList() {
  ...
  const handleAddToCart = (plant) => {
    ...
    dispatch(addItem({ ...plant, quantity })); // Dispatch action to add the plant to the cart
 ...
 const handleRemoveFromCart = (plantName) => {
  dispatch(removeItem(plantName)); // Dispatch action to remove the plant from the cart

  // Update the state immutably to trigger a re-render
  setAddedToCart((prevState) => ({
    ...prevState,
    [plantName]: false, // Set the item to false, allowing it to be added again
  }));

};

useEffect(() => {
  // Update the addedToCart state based on cart items
  const updatedAddedToCart = {};
  cart.forEach((item) => {
    updatedAddedToCart[item.name] = true; // Mark all items in the cart as added
  });
  setAddedToCart(updatedAddedToCart); // Update the state with the new object
}, [cart]);
  const plantsArray = [
    {
  ...
  // Handle clicking on the Plants button
  ...
  // Handle continue shopping button in the cart view
    ...
    <div>
      <div className="navbar" style={styleObj}>
        ...
        <div style={styleObjUl}>
          ...
          <span className="cart-quantity">{calculateTotalItems()}</span>
        )}
          </h1></a></div>
        </div>
      </div>

      {/* Show Plants or Cart based on state */}
      {showPlants && (
  ...
  <CartItem 
    key={index} 
    item={item} 
    handleRemoveFromCart={handleRemoveFromCart} // Pass the function here
  />
))}
