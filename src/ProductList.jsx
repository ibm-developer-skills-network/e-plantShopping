...
const ProductList=() =>{
    ...
    const name =cart.map((res,index)=> res.name)
    const [showProductList, setShowProductList] = useState(false);

    const handleGetStartedClick = (z) => {
      setShowProductList(z);
    };
    // console.log(Object.keys(addedToCart))
    // console.log(plantsArray)
    // Add to Cart
    // const {addItem} = CreatSlice
    ...
    // console.log(cart)
    // console.log(plantsArray.map((category,index)=> (
    //     category.plants.map((res) => (
    //         console.log(cart.map((resz)=> resz.name===res.name))
    //     ))
    // )))
   ...
   const styleA={
    ...
    return (
        ...
        <CartItem prantCallBack={handleGetStartedClick}/>
      </div>
            ...
            <div onClick={handleGetStartedClick} style={styleObjUl}>
                ...
                <div> <a href="#" style={styleA}><h1 className='cart'>
                    ...
                    </h1><span className='span'>{cart.length}</span></a></div>
            </div>
        </div>
                ...
                <div className="product-list">
                  {category.plants.map((plant, plantIndex) => (
                    <div className="product-card" key={plantIndex}>
                     ...
                     <button disabled={name.includes(plant.name)} onClick={() => handleAddToCart(plant)} 
                    >{name.includes(plant.name)?"Added to cart":"Add to cart"}</button>
                    </div>
                  ))}
                </div>
              </div>
