...
function ProductList() {
  const [showCart, setShowCart] = useState(false); // Toggle between cart and plants grid
  const dispatch = useDispatch(); // Correctly declare dispatch once
  ...
  const handlePlantsClick = (e) => {
    e.preventDefault();
    setShowPlants(true); // Show the plants grid
    setShowCart(false); // Hide the cart
  };

  const handleContinueShopping = (e) => {
    e.preventDefault();
    setShowCart(false); // Return to the plants grid
  };

  const handleAddToCart = (plant) => {
    dispatch(addItem(plant)); // Dispatch Redux action to add plant to the cart
  };
